...
class M_detail_order extends CI_Model
{
    ...
    function get_detail_menu($id, $menu)
    {
        $sql = "SELECT 
                * 
                FROM dat_detail_order
                WHERE order_id = ?
                AND menu_id=?";
        $query = $this->db->query($sql, array($id, $menu));
        $result = $query->row_array();
        return $result;
    }
    function create_detail_order()
    {
        ...
        $cek = $this->get_detail_menu($data['order_id'], $data['menu_id']);
        if ($cek) {
            ...
            $this->db->where('detail_order_id', $cek['detail_order_id']);
